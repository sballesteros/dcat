...
function publish(pkg, callback){
  ...
  self.publishDataset(pkg.dataset, function(err){
    if(err) return callback(err);
    self.publishCode(pkg.code, function(err){
      if(err) return callback(err);
      self.publishFigure(pkg.figure, function(err){
        if(err) return callback(err);
        self.publishArticle(pkg.article, function(err){
          if(err) return callback(err);
          ...
          _getPkgStream.call(self, pkg, function(err, pkgStream){
            if(err) return callback(err);
    ...
    if ( ('distribution' in x) && ('contentPath' in x.distribution) ) {
      ...
      fs.stat(p, function(err, stat){
        ...
        s.on('end', function() {
          ...
          self.logHttp('PUT', rurl);
          var r = request.put( { url: rurl, auth: self.auth(), headers: headers }, _cbrequest(self, cb));
  ...
  async.each(code, function(x, cb){
    ...
    if( ('targetProduct' in x) && ('filePath' in x.targetProduct) ) {
      ...
      if(!x.targetProduct.fileFormat) {
        x.targetProduct.fileFormat = mime.lookup(x.targetProduct.fileFormat);
      }
