...
function publish(pkg, attachments, callback){
  ...
  publishDataset.call(self, pkg.dataset, function(err){
    ...
    publishCode.call(self, pkg.code, function(err){
      if(err) return callback(err);
      publishMediaObject.call(self, pkg.figure, function(err){
        if(err) return callback(err);
        publishMediaObject.call(self, pkg.audio, function(err){
          if(err) return callback(err);
          publishMediaObject.call(self, pkg.video, function(err){
            if(err) return callback(err);
  ...
  async.each(dataset, function(x, cb){
    ...
    if ( ('distribution' in x) && ('contentPath' in x.distribution) ) {
      ...
      fs.stat(p, function(err, stat){
        if(err) {
          return _skipIfUrlExists.call(self, err, x.distribution.contentUrl, cb);
        }
        ...
        s.on('end', function() {
          x.distribution.encoding = { encodingFormat: 'gzip', hashAlgorithm: 'sha1', hashValue: sha1.digest('hex'), contentSize: size };
          x.distribution.contentUrl = 'r/' + x.distribution.encoding.hashValue;

          var headers = {
            'Content-Length': x.distribution.encoding.contentSize,
            'Content-Type': x.distribution.encodingFormat,
            'Content-Encoding': 'gzip'
          };

          var rurl = self.url('/r/' + x.distribution.encoding.hashValue);
          self.logHttp('PUT', rurl);
...
function publishMediaObject(media, callback){
  ...
  async.each(media, function(x, cb){
    ...
    if ('contentPath' in x) {
      ...
      fs.stat(p, function(err, stat){
        if(err) {
          return _skipIfUrlExists.call(self, err, x.contentUrl, cb);
        }
...
function publishArticle(article, callback){
  ...
  async.each(article, function(x, cb){
    ...
    if ( ('encoding' in x) && ('contentPath' in x.encoding) ) {
      ...
      fs.stat(p, function(err, stat){
        if(err) {
          return _skipIfUrlExists.call(self, err, x.encoding.contentUrl, cb);
        }
...
//        x.targetProduct.fileFormat = mime.lookup(x.targetProduct.filePath); //TODO fix maybe set to text/plain
      ...
      fs.stat(p, function(err, stat){
        if(err) {
          return _skipIfUrlExists.call(self, err, x.targetProduct.downloadUrl, cb);
        }
