  ...
  publishThumbnail.call(that, pkg, function(err){ //DO first because first check that thumbnailPath are indeed images..
    ...
    fs.stat(path.resolve(that.root, 'README.md'), function(err, stat){
      ...
      publishDataset.call(that, pkg.dataset, function(err){
        if(err) return callback(err);
        publishCode.call(that, pkg.code, function(err){
          if(err) return callback(err);
          publishMediaObject.call(that, pkg.figure, function(err){
            if(err) return callback(err);
...
/**
 * 'this' is an Ldpm instance
 * publish MediaObject (figure, audio, video, article) AND edit in place to add metadata
 */
...
function publishThumbnail(pkg, callback){
  ...
  var allTypes = ['dataset', 'code', 'figure', 'audio', 'video', 'article']
...
/**
 * 'this' is an Ldpm instance
 * publish code AND edit code in place to add metadata AND delete filePath if codeBundle
 */
