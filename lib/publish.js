...
function _getDpkgStream(callback){
  ...
  fs.readFile(path.resolve(root, 'datapackage.jsonld'), function(err, doc){
    ...
    ignore.addIgnoreRules(dataPaths.concat(['package.json', 'datapackage.json', 'datapackage.jsonld', '.data', '.git', '__MACOSX', 'datapackages', 'node_modules', 'README.md']), 'custom-rules');
