...
function _getDpkgStream(callback){
  ...
  fs.readFile(path.resolve(root, 'datapackage.json'), function(err, doc){
    if(err) return callback(err);
    ...
    var data = doc.dataset.filter(function(x){return 'path' in x});

    var dataPaths = data.map(function(x){return x.path});
    var dataNames = data.map(function(x){return x.name});
    
    //compress everything (not ignored) but the data and the datapackage.json
    var ignore = new Ignore({
      path: root,
      ignoreFiles: ['.gitignore', '.npmignore', '.ldpmignore'].map(function(x){return path.resolve(root, x)})
    });
    ignore.addIgnoreRules(dataPaths.concat(['package.json', 'datapackage.json', '.data', '.git', 'datapackages', 'node_modules', 'README.md']), 'custom-rules');
