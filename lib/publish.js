var path = require('path')
  ...
  , clone = require('clone')
  , fs = require('fs')
  , addToMaintainer = require('./addToMaintainer')
    ...
    var req = http.request(_updOptions(options, dpkgStream), function(res){
      ...
      res.on('end', function(){
        ...
        if(code === 201){
          ...
          addToMaintainer({granter: this.rc.name, granted: this.rc.name, dpkgName: dpkgStream._name, _id: dpkgStream._id}, callback);
...
function _updOptions(options, dpkgStream){
  ...
  options.path = '/stan/' + dpkgStream._id;
