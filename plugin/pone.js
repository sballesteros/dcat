...
function _extractAuthors(document){
  ...
  var auth = document.getElementsByClassName('email')[0].parentNode.parentNode.parentNode.parentNode.innerHTML;

  authors = [];
  Array.prototype.forEach.call(document.getElementsByClassName('author'),
    function(x){
      var author = {};
      var tmp = x.textContent;
      tmp = tmp.trim();
      tmp = tmp.split('\n')[0];
      tmp = tmp.replace(',','');
      author.name = tmp.trim();
      tmp = x.parentNode.innerHTML;
      if(tmp.indexOf('Affiliations')>-1){
        tmp = _extractBetween(tmp,'Affiliations:','</p>').trim().split('\n');
      } else {
        tmp = _extractBetween(tmp,'Affiliation:','</p>').trim().split('\n');
      }
      tmp.forEach(function(x,i){
        tmp[i] = x.trim();
        if(x[x.length-2]==','){
          tmp[i] = x.slice(0,x.length-2);
        }
      })
...
function _extractAbstract(document){
  var $abstract = document.getElementsByClassName('abstract')[0];
  var tmp = '';
  if($abstract!=undefined){
    tmp = $abstract.innerHTML;
    while(tmp.indexOf('<a id=')>-1){
      tmp = tmp.slice(0,tmp.indexOf('<a id=')) + tmp.slice(tmp.indexOf('<a id=') + tmp.slice(tmp.indexOf('<a id='),tmp.length).indexOf('</a>')+4,tmp.length);
    }
    if(tmp.slice(0,17)=='<h2>Abstract</h2>'){
      tmp = tmp.slice(17);
    }
  }
  return tmp;
...
function _extractFigures(document,doi){

  var figures = [];

  Array.prototype.forEach.call(document.getElementsByClassName('figure'),function(x){
    var figure = {};
    var tmp = '';

    figure.id = x.id;
    figure.link = "http://www.plosone.org/article/info:doi/"+doi+'.'+x.id.split('-')[2]+'/'+'originalimage';

    Array.prototype.forEach.call(x.getElementsByTagName('p'),function(p,i){
      if(i==0){
        figure.name = p.textContent;
      } else {
        tmp += p.textContent + ' ';
      }
    });
...
function _extractSupplementary(document){

  var supplementary = [];
  var tmp = document.getElementsByClassName('siTitle')[0];

  ind = 0;

  if(tmp!=undefined){
    tmp = tmp.parentNode;
    Array.prototype.forEach.call(tmp.getElementsByTagName('p'),function(x,i){
      var si  = {};

      if(x.className=='siTitle'){
        supplementary.push(si);
        supplementary[ind].name = x.textContent.trim();
        // article['supplementary'][ind].link = 'http://www.plosone.org/' + x.getElementsByTagName('a')[0].href.slice(8);
      }
      if(x.className=='preSiDOI'){
        supplementary[ind].caption = x.textContent;
      }
      if(x.className=='siDoi'){
        supplementary[ind].doi = x.textContent.slice(4,x.length);
        supplementary[ind].link = "http://www.plosone.org/article/info:doi/"+supplementary[ind].doi;
      }
      if(x.className=='postSiDOI'){
        supplementary[ind].format = x.textContent.slice(1,x.textContent.length-1);
        if(supplementary[ind].format.indexOf('TIF')>-1){
          supplementary[ind].link += '/originalimage';
        }
        ind += 1;
      }
    });
...
function _extractDatePublished(document,year){
  var beg = document.innerHTML.indexOf('<ul class="date-doi-line">');
  beg = beg + document.innerHTML.slice(beg,document.innerHTML.length).indexOf('Published:')+10;
  var end = beg + document.innerHTML.slice(beg,document.innerHTML.length).indexOf('</li>');
  var tmp = document.innerHTML.slice(beg,end).trim();
  var months = {
    January: '01',
    February: '02',
    March: '03',
  ...
  var month = months[tmp.split(' ')[0]];
  var day = tmp.split(' ')[1].slice(0,2);

  return year + '-' + month + '-' + day;
}

function _extractCitations(document){
  citation = [];
  var $refs = document.getElementsByClassName('references')[0];

  Array.prototype.forEach.call($refs.childNodes,function(x,i){
    var ref = {};
    var beg = x.innerHTML.indexOf('</a>')+4;
    var end = beg + x.innerHTML.slice(beg).indexOf('<ul');
    var tmp = x.innerHTML.slice(beg,end).replace('\n','').trim();
    var ind = x.innerHTML.indexOf('Go to article in PubMed');

    ref.description = tmp;
    if(tmp.indexOf('doi:')>-1){
      beg = ref.description.indexOf('doi:')+4;
      if(ref.description[ref.description.length-1]=='.'){
        end = ref.description.length -1;
      } else {
        end = ref.description.length;
      }
      ref.doi = ref.description.slice(beg,end).trim();
...
function _extractPdfUrl(document){
  var tmp = document.getElementsByClassName('download')[0].innerHTML;
  return _extractBetween(tmp,'href="','"');
}
...
function _scrap(uri,body){
  ...
  var author = document.getElementsByClassName('email')[0].parentNode.parentNode.parentNode.parentNode.innerHTML;
  var contact = {};
  var article = {};
...
function _initPkg(uri,article){
  ...
  organisations = _.uniq(organisations);
  pkg.sourceOrganization = [];
  organisations.forEach(function(o){
    pkg.sourceOrganization.push({ name : o })
  });
...
function _wrapResources(article,resources){

  if(resources!=undefined){

    article.figures.concat(article.supplementary).forEach(function(x){
      var found = false;
